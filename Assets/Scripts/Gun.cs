...
public class Gun : MonoBehaviour {
    ...
    public static Gun Instance
    {
        get
        {
            if(instance == null)
            {
                instance = FindObjectOfType<Gun>();
                if(instance == null)
                {
                    GameObject container = new GameObject("Gun");

                    instance = container.AddComponent<Gun>();
                }
            }
            return instance;
        }
    ...
    private void Update()
    {
        ...
        bulletLineRenderer.SetPosition(1, fireTransform.forward * fireDistance);
        // 라인 렌더러를 활성화하여 총알 궤적을 그린다
        UIManager.instance.UpdateGunPowerText(damage);
    }
    private void Awake() {
        // 사용할 컴포넌트들의 참조를 가져오기
    ...
    private IEnumerator ShotEffect(Vector3 hitPosition) {
        ...
        bulletLineRenderer.material.color =Color.red;
        // 0.03초 동안 잠시 처리를 대기
        yield return new WaitForSeconds(0.1f);

        // 라인 렌더러를 비활성화하여 총알 궤적을 지운다
        bulletLineRenderer.material.color = Color.green;
    }
